...
static void buildMatrix(const char *keyword, char matrix[5][5]) {
    ...
    for (int i = 0; i < 25; ++i) matrix[i / 5][i % 5] = temp[i];
}
...
int main(void) {
    char keyword[100] = "MONARCHY";
    char plaintext[1000] = "MUST SEE YOU OVER CADOGAN WEST COMING AT ONCE BRO GIVE ME CODE";
    char prepared[1000];
    char matrix[5][5];
